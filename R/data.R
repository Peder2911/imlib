...
#' @param variables A list of variables. The names should correspond to the names of variables in the dataset, while the values should be the values you want the variables to have. 
#' @param avgfun Function to produce placeholder values for variables not included in the variables list.
#' @export
makeTestSet <- function(data,variables,avgfun){
   ...
   if(!all(nrow(out) == tgtShape$rows, ncol(out) == tgtShape$columns)){
      stop(paste("Wrong number of dimensions:",dim(out),dim(tgtShape)))
   }
